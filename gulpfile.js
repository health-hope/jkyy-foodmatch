...
gulp.task('test_copy_font', function () {
    return gulp.src(['src/css/fontcss/*.*'])
        .pipe(gulp.dest(test_outdir + '/css/fontcss'))
        .pipe(browserSync.stream());
});
 ...
 * 复制css libs
 */
gulp.task('test_copy_css', function () {
    return gulp.src(['src/sass/libs/*.css'])
        .pipe(gulp.dest(test_outdir + '/css/libs'))
        .pipe(browserSync.stream());
});
...
gulp.task('test_img', function () {
    return gulp.src(["src/image/*.*", "src/image/**/*.*"])
        // .pipe(imagemin({
        //     progressive: true
        // }))
        .pipe(gulp.dest(test_outdir + "/image"))
        .pipe(browserSync.stream());
});
 ...
 * 创建本地开发服务
 */
gulp.task('dev', ['test_js', 'test_copyjs', 'test_copy_font', 'test_copy_css','test_img', 'test_html', 'test_sass', 'test_config'], function () {
    browserSync.init({
        server: './'
    });
    gulp.watch("src/js/*.js", ['test_js']);
    gulp.watch(["src/sass/*.scss"], ['test_sass']);
    gulp.watch(["src/image/*", "src/images/**/*"], ['test_img']);
    gulp.watch(["src/views/*.html", "src/*.html"], ['test_html']);
});
/**
 * 创建测试环境项目
 */
gulp.task('build', ['test_js', 'test_copyjs', 'test_copy_font', 'test_copy_css', 'test_img', 'test_html', 'test_sass', 'test_config'], function () {
});
